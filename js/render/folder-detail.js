...
function viewFolder(){
  ...
  /* A top-level folder is a category — Client Calls, Projects — not a place
     ...
     always read "0 meetings" here. */
  const fs=unfiled?'<span>Not in a folder yet</span>'
    :isTop?`<span><b>${kids.length}</b> folder${kids.length===1?'':'s'}</span>`
    :`<span><b>${all.length}</b> meeting${all.length===1?'':'s'}</span>${
        tasks.length?`<span class="sp"></span><span><b>${tasks.length}</b> open task${
  ...
  const {shown:mrows,more:mmore}=capList('folder:'+id+':meetings',all.map(m=>meetingRow(m)),8);
  const mlist=pnl('Meetings',`${all.length} here`,'',
    all.length?`<div class="list">${mrows.join('')}</div>${mmore}`
      :`<div class="hint" style="padding:var(--s2) 2px var(--s3)">Nothing filed here yet.</div>`,true);

  return `<div class="body${S.mpanel?'':' docked'}">${head}${band}${roll}${mlist}
...
function briefBlock(id){
  ...
  return `<div class="brief">
    <div class="bsec lead"><h4>Where things stand</h4>
      <p class="flead">${esc(b.stand)}</p></div>
    <div class="bgrid">
...
/* Decisions, open questions and open tasks, read across every meeting filed in
   the folder. */
function rollup(id){
  const dec=folderLines(id,'decisions'),qs=folderLines(id,'questions'),tasks=folderTasks(id);
  ...
  return `<div class="rollgrid" style="margin-bottom:var(--s7)">
    ...
    ${!dec.length&&!qs.length?`<div class="sec"><p style="color:var(--sec)">No decisions or open
      questions have come out of these calls yet.</p></div>`:''}
    ${secBlock({key:'roll:'+id+':tasks',title:'Open tasks',rows:tasks.map(t=>taskRow(t)),
      action:tasks.length?`<span data-a="go" data-p="tasks">View in Tasks ${ic('chev',12)}</span>`:'',
      empty:'Nothing open from this folder.'})}
    </div>`;
}
